        ...
        <div className="flex gap-2">
          {snaps.map((_, i) => (
            <button
              ...
              className={`h-2 rounded-full transition-all ${i === selected ? "w-8 bg-[var(--brand-green)]" : "w-2 bg-muted-foreground/40"}`}
            />
          ))}
        </div>
        <div className="flex gap-2">
          <button onClick={prev} aria-label="Anterior" className="flex h-10 w-10 items-center justify-center rounded-full bg-card shadow-card hover:bg-[var(--brand-yellow)] hover:text-accent-foreground transition-colors group">
            <ChevronLeft className="h-5 w-5 group-hover:text-[var(--brand-green)]" />
          </button>
          <button onClick={next} aria-label="Próximo" className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--brand-green)] text-primary-foreground shadow-brand hover:opacity-90 transition-opacity">
